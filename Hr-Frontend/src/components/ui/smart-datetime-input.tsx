   ...
   React.useEffect(() => {
      const getCurrentElementTime = () => {
         ...
         for (let j = 0; j <= 3; j++) {
            ...
            const selected =
               !hour12 ||
               (PM_AM === (formattedHours >= 12 ? 'PM' : 'AM') && (minutes <= 53 ? diff < Math.ceil(timestamp / 2) : diff < timestamp));
...
>(({ locale, hour12, placeholder, ...props }, ref) => {
   ...
   const _placeholder =
      placeholder ??
      (locale.code.split('-')[0] === 'fr' ? 'e.g. "demain à 17h" ou "dans 2 heures"' : 'e.g. "tomorrow at 5pm" or "in 2 hours"');
