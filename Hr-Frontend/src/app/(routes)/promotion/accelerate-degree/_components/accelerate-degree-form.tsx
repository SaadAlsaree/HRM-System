...
const formSchema = z.object({
   ...
   lengthOfService: z.string().min(1, 'مدة الخدمة مطلوب'),
...
const AccelerateAchievementForm = ({ data, icon, title, variant }: Props) => {
   ...
   const form = useForm<z.infer<typeof formSchema>>({
      ...
      defaultValues: {
         ...
         jobTitleToId: data?.jobTitleToId?.toString() ?? '',
         jobDescriptionToId: data?.jobDescriptionToId?.toString() ?? '',
         jobDegreeToId: data?.jobDegreeToId?.toString() ?? '',
         jobCategoryToId: data?.jobCategoryToId?.toString() ?? '',
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='grid grid-cols-3 gap-4 py-4' autoComplete='off'>
                     ...
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>مدة الخدمة</FormLabel>
                              <Input placeholder='مدة الخدمة' type='text' {...field} />
                           </FormItem>
                        )}
                     />
