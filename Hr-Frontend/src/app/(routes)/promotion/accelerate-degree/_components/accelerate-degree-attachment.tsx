...
const AccelerateDegreeAttachment = ({ employeeId, PrimaryTableId }: Props) => {
    ...
    const handleSubmit: (attachment: IAttachments) => Promise<void> = async (attachment: IAttachments) => {
       ...
       try {
          const response = await changeDegreeService.uploadAttachment(formData);
          if (response) {
             toast(
...
export default AccelerateDegreeAttachment;
