...
const LeavesForm = ({ data, icon, title, variant, countryData }: Props) => {
   ...
   const onSubmit = async (values: z.infer<typeof formSchema>) => {
      ...
      try {
         if (data) {
            const dataToUpdate = { ...values, employeeId: data.employeeId, typeOfLeaveId: 5 };
            await leavesService.updateLeave(data.id, dataToUpdate);
            toast.success('تم تعديل البيانات بنجاح.');
         } else {
            const dataToCreate = { ...values, employeeId: selectedUser?.employeeId, typeOfLeaveId: 5, createBy: 'user-id' };
                     ...
                     <div className='grid grid-cols-12 gap-4'>
                        ...
                        <div className='col-span-6'>
                           <FormField
                              ...
                              name='isInside'
                              render={({ field }) => (
                                 <FormItem className='flex flex-row items-start space-x-3 mt-6 space-y-0 rounded-md border p-4'>
                                    <FormControl>
