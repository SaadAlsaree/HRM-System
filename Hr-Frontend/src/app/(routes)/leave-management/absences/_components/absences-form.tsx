...
const formSchema = z.object({
   ...
   absenceDate: z.string().min(1, 'تاريخ الغياب مطلوب'),
   countOfDays: z.number().min(1, 'عدد الأيام يجب أن يكون على الأقل 1'),
   absenceOrderNo: z.string().optional(),
   absenceOrderDate: z.string().optional(),
   returnOrderNo: z.string().optional(),
   returnOrderDate: z.string().optional(),
...
const AbsencesForm = ({ data, icon, title, variant }: Props) => {
   ...
   const form = useForm<z.infer<typeof formSchema>>({
      ...
      defaultValues: {
         bookNo: data?.bookNo ?? '',
         bookDate: data && data.bookDate ? new Date(data.bookDate).toISOString().split('T')[0] : '',
         absenceDate: data && data.absenceDate ? new Date(data.absenceDate).toISOString().split('T')[0] : '',
   ...
   const onSubmit = async (values: z.infer<typeof formSchema>) => {
      ...
      try {
         if (data) {
            ...
            await absencesService.putAbsenceById(data.id, dataToUpdate);
            toast.success('تم تعديل البيانات بنجاح.');
         } else {
            const dataToCreate = { ...values, employeeId: selectedUser?.employeeId, createBy: 'user-id' };
            await absencesService.createAbsence(dataToCreate);
                     ...
                     <div className='grid grid-cols-1 gap-4 xl:grid-cols-12'>
                        ...
                        <div className='col-span-6'>
                           <FormField
                              ...
                              render={({ field }) => (
                                 <FormItem>
                                    <FormLabel>تاريخ الغياب</FormLabel>
                                    <FormControl>
                                       <Input placeholder='تاريخ الغياب' type='date' {...field} />
                                    </FormControl>
                                    <FormMessage />
                                 </FormItem>
                              )}
                        ...
                        <div className='col-span-6'>
                           <FormField
                              ...
                              render={({ field }) => (
                                 <FormItem>
                                    <FormLabel>امر أداري بالغياب</FormLabel>
                                    <FormControl>
                                       <Input placeholder='امر أداري بالغياب' type='text' {...field} />
                                    </FormControl>
                                    <FormMessage />
                                 </FormItem>
                              )}
                        ...
                        <div className='col-span-6'>
                           <FormField
                              control={form.control}
                              name='absenceOrderDate'
                              render={({ field }) => (
                                 <FormItem>
                                    <FormLabel>تاريخ الأمر الإداري بالغياب</FormLabel>
                                    <FormControl>
                                       <Input placeholder='تاريخ الأمر الإداري بالغياب' type='date' {...field} />
                                    </FormControl>
                                    <FormMessage />
                                 </FormItem>
                              )}
                        ...
                        <div className='col-span-6'>
                           <FormField
                              ...
                              render={({ field }) => (
                                 <FormItem>
                                    <FormLabel>رقم الأمر الإداري بالعودة</FormLabel>
                                    <FormControl>
                                       <Input placeholder='رقم الأمر الإداري بالعودة' type='text' {...field} />
                                    </FormControl>
                                    <FormMessage />
                                 </FormItem>
                              )}
                        ...
                        <div className='col-span-6'>
                           <FormField
                              ...
                              render={({ field }) => (
                                 <FormItem>
                                    <FormLabel>تاريخ الأمر الإداري بالعودة</FormLabel>
                                    <FormControl>
                                       <Input placeholder='تاريخ الأمر الإداري بالعودة' type='date' {...field} />
                                    </FormControl>
                                    <FormMessage />
                                 </FormItem>
                              )}
