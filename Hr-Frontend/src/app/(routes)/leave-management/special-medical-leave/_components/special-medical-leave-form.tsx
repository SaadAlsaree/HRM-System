...
type Props = {
   data?: ISpecialMedicalLeave;
   icon?: React.ReactNode;
   title: string;
   variant?: 'ghost' | 'outline' | 'default' | 'destructive' | 'link';
};

const SpecialMedicalLeaveForm = ({ data, icon, title, variant }: Props) => {
   ...
   const onSubmit = async (values: z.infer<typeof formSchema>) => {
      ...
      try {
         if (data) {
            const dataToUpdate = { ...values, employeeId: data.employeeId, typeOfLeaveId: 7 };
...
export default SpecialMedicalLeaveForm;
