...
import AppointmentLeaveAttachments from './appointment-leave-attachments';
import moment from 'moment';
import { Button } from '@/components/ui/button';

type Props = {
   columns: { label: string; value: string; className?: string }[];
   appointmentLeaveData: IAppointmentLeave[];
};

const AppointmentLeaveTable = ({ appointmentLeaveData, columns }: Props) => {
               ...
               <TableBody>
                  {appointmentLeaveData?.map((item) => (
                     <TableRow key={item.id}>
                        ...
                        <TableCell>
                           <AppointmentLeaveAttachments PrimaryTableId={item.id} employeeId={item.employeeId} />
                        </TableCell>
                        <TableCell>
                           <Popover>
                        ...
                        <TableCell>
                           <AppointmentLeaveForm title='' data={item} icon={<Settings2 />} variant='ghost' />
                        </TableCell>
                     </TableRow>
                  ))}
               </TableBody>
...
export default AppointmentLeaveTable;
