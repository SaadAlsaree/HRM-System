...
const AppointmentLeavePage = async ({ searchParams }: Props) => {
   ...
   const data = await fetchServer<{ items?: IAppointmentLeave[]; totalCount?: number; data?: { items?: IAppointmentLeave[]; totalCount?: number } }>('/Leave', 'GET', { params: { Page, PageSize, TypeOfLeaveId: 8, status: 1 } });
   const appointmentLeaveData = (data?.items ?? data?.data?.items) ?? [];
