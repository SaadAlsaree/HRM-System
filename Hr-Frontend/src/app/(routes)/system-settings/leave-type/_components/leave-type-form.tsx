...
const LeaveTypeForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await typeOfLeaveService.updateTypeOfLeave(data.id, values);
            toast(
            ...
            await typeOfLeaveService.createTypeOfLeave(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نوع الأجازة</FormLabel>
                              <FormControl>
                                 <Input placeholder='الأجازة' type='text' {...field} />
                              </FormControl>
