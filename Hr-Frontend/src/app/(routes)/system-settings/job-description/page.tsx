...
const JobDescriptionPage = async ({ searchParams }: Props) => {
   ...
   const data = await fetchServer<{ items?: any[]; totalCount?: number; data?: { items?: any[]; totalCount?: number } }>('/JobDescription', 'GET', { params: { Page, PageSize } });
   const jobDescriptions = (data?.items ?? data?.data?.items) ?? [];
