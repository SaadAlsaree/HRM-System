...
import { IJobDescription } from '../page';
...
const JobDescriptionForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await jobDescriptionService.updateJobDescription(data.id, values);
            toast(
            ...
            await jobDescriptionService.createJobDescription(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>أسم الوصف الوظيفي</FormLabel>
                              <FormControl>
                                 <Input placeholder='أسم الوصف الوظيفي' type='text' {...field} />
                              </FormControl>
