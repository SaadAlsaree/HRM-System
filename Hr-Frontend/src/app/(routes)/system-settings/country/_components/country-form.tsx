...
const CountryForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await countryService.updateCountry(data.id, values.name);
            toast(
            ...
            await countryService.createCountry(values.name);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>أسم البلد</FormLabel>
                              <FormControl>
                                 <Input placeholder='البلد' type='text' {...field} />
                              </FormControl>
