...
import { IEmployeeDocumentType } from '../page';
...
type Props = {
   data?: IEmployeeDocumentType;
   icon?: React.ReactNode;
   title: string;
   variant?: 'ghost' | 'outline' | 'default' | 'destructive' | 'link';
};

const EmployeeDocumentTypeForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await employeeDocumentsTypeService.updateEmployeeDocumentsTypeById(data.id, values);
            toast(
            ...
            await employeeDocumentsTypeService.createEmployeeDocumentsType(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نوع مستمسكات الموظف</FormLabel>
                              <FormControl>
                                 <Input placeholder='نوع مستمسكات الموظف' type='text' {...field} />
                              </FormControl>
