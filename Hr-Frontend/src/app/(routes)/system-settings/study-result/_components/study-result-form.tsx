...
const StudyResultForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await studyResultService.updateStudyResult(data.id, values);
            toast(
            ...
            await studyResultService.createStudyResult(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نتيجة الدراسة</FormLabel>
                              <FormControl>
                                 <Input placeholder='نتيجة الدراسة' type='text' {...field} />
                              </FormControl>
