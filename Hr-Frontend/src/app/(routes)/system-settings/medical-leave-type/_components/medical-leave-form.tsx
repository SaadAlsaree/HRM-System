...
import { IMedicalLeaveType } from '../page';
...
type Props = {
   data?: IMedicalLeaveType;
   icon?: React.ReactNode;
   title: string;
   variant?: 'ghost' | 'outline' | 'default' | 'destructive' | 'link';
};

const MedicalLeaveForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await sicknessTypeService.updateSicknessType(data.id, values);
            toast(
            ...
            await sicknessTypeService.createSicknessType(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نوع الأجازة المرضية</FormLabel>
                              <FormControl>
                                 <Input placeholder='الأجازة المرضية' type='text' {...field} />
                              </FormControl>
