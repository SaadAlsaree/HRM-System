...
import { ICurrentPosition } from '../page';
import CurrentPositionForm from './current-position-form';
import { employeePositionService } from '@/services/Employee/employee-position.service';

type Props = {
    columns: { label: string; value: string; className?: string }[];
    currentPositionData: ICurrentPosition[];
};

const CurrentPositionTable = ({ currentPositionData, columns }: Props) => {
                       ...
                       <TableCell>{item.jobCode}</TableCell>
                        ...
                        <TableCell>
                            <div className='flex items-center gap-2'>
                                ...
                                <CurrentPositionForm title='' icon={<Settings2 className='h-4 w-4' />} data={item} variant='ghost' />
                            </div>
                        </TableCell>
                        <TableCell>
