...
import InterruptionsAttachment from './interruptions-attachment';
import InterruptionsForm from './interruptions-form';

type Props = {
   columns: { label: string; value: string; className?: string }[];
   resignationList: IInterruptions[];
};

const InterruptionsTable = ({ columns, resignationList }: Props) => {
   const router = useRouter();
   //Handel Update status
   const handleStatusChange = async (value: string | number | null, id: string | number | null) => {
      try {
         const response = await interruptionService.patchInterruption({ id, statusId: value });
               ...
               <TableBody>
                  {resignationList?.map((item) => (
                     <TableRow key={item?.id}>
                        ...
                        <TableCell>{item?.notificationDate?.split('T', 1)}</TableCell>
                        <TableCell>{item?.reason}</TableCell>
                        <TableCell>
                           <SelectStatus id={item?.id as string} status={item?.status?.toString()} onChange={handleStatusChange} />
                        </TableCell>
                        <TableCell>
                           <InterruptionsAttachment PrimaryTableId={item?.id as string} employeeId={item?.employeeId as string} />
                        </TableCell>
                        <TableCell>
                           <Popover>
                        ...
                        <TableCell>
                           <InterruptionsForm title='' data={item} icon={<Settings2 />} variant='ghost' />
                        </TableCell>
                     </TableRow>
                  ))}
               </TableBody>
