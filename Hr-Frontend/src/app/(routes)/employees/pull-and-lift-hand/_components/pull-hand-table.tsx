...
import PullHandAttachment from './pull-hand-attachment';
import moment from 'moment';

type Props = {
   columns: { label: string; value: string; className?: string }[];
   handPullList: IHandPull[];
};
const PullHandTable = ({ columns, handPullList }: Props) => {
   const router = useRouter();
   //Handel Update status
   const handleStatusChange = async (value: string | number | null, id: string | number | null) => {
      try {
         const response = await handPullService.patchHandPull({ id, statusId: value });
      ...
      <div>
         <ScrollArea className='w-[1380px] whitespace-nowrap '>
            <Table>
               ...
               <TableBody>
                  {handPullList?.map((item) => (
                     <TableRow key={item?.id}>
                        <TableCell>{item?.id?.toString().toUpperCase().split('-', 1)}</TableCell>
                        <TableCell>{item?.fullName}</TableCell>
                        <TableCell>{item?.lotNumber}</TableCell>
                        <TableCell>{item?.jobCode}</TableCell>
                        <TableCell>{item?.withdrawHandPullOrderNo}</TableCell>
                        <TableCell>{moment(item?.withdrawHandPullOrderDate).format('DD/MM/YYYY')}</TableCell>
                        <TableCell>{item?.raiseHandPullOrderNo}</TableCell>
                        <TableCell>{moment(item?.raiseHandPullOrderDate).format('DD/MM/YYYY')}</TableCell>
                        <TableCell>
                           <SelectStatus id={item?.id as string} status={item?.status?.toString()} onChange={handleStatusChange} />
                        </TableCell>
                        <TableCell>
                           <PullHandAttachment PrimaryTableId={item?.id as string} employeeId={item?.employeeId as string} />
                        </TableCell>
                        <TableCell>
                           <Popover>
                        ...
                        <TableCell>
                           <PullHandForm title='' data={item} icon={<Settings2 />} variant='ghost' />
                        </TableCell>
                     </TableRow>
                  ))}
               </TableBody>
