...
const MartyrsAndWoundedPage = async ({ searchParams }: Props) => {
   ...
   const data = await fetchServer<{ items?: IMartyrsAndWounded[]; totalCount?: number; data?: { items?: IMartyrsAndWounded[]; totalCount?: number } }>(
      '/MartyrsAndWounded',
      'GET',
      {
   ...
   const martyrsWoundedList: IMartyrsAndWounded[] = (data?.items ?? data?.data?.items) ?? [];
