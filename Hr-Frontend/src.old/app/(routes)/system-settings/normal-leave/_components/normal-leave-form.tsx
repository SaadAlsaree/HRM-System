...
const NormalLeaveForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await normalLeaveTypeService.updateNormalLeaveType(data.id, values);
            toast(
            ...
            await normalLeaveTypeService.createNormalLeaveType(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نوع الأجازة الأعتيادية</FormLabel>
                              <FormControl>
                                 <Input placeholder='الأجازة الأعتيادية' type='text' {...field} />
                              </FormControl>
