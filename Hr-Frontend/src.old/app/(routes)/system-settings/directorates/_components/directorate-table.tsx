...
type Props = {
   ...
   DirectorateData: IDirectorate[];
   data?: org[];
};

const DirectorateTable = ({ columns, DirectorateData, data }: Props) => {
   const router = useRouter();
   //Handel Update status
   const handleStatusChange = async (value: string | number | null, id: string | number | null) => {
      try {
         const response = await subDirectorService.patchSubDirectorates({ id, statusId: value });
               ...
               <TableRow key={item.id}>
                  ...
                  <TableCell>
                     <div className='flex items-center gap-2'>
                        <DirectorateForm title='' icon={<Settings2 className='h-4 w-4' />} data={item} variant='ghost' orgData={data} />
                     </div>
                  </TableCell>
               </TableRow>
            ))}
