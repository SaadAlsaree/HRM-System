...
const AppointmentTypeFrom = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await typeOfJobService.updateTypeOfJob(data.id, values);
            toast(
            ...
            await typeOfJobService.createTypeOfJob(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نوع التعين</FormLabel>
                              <FormControl>
                                 <Input placeholder='نوع التعين' type='text' {...field} />
                              </FormControl>
...
export default AppointmentTypeFrom;
