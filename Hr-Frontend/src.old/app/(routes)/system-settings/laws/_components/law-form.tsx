...
const LawForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await lawService.updateLaw(data.id, values);
            toast(
            ...
            await lawService.createLaw(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>القانون</FormLabel>
                              <FormControl>
                                 <Input placeholder='القانون' type='text' {...field} />
                              </FormControl>
