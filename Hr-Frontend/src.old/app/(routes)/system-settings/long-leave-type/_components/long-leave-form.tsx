...
import { ILongLeaveType } from '../page';
...
const LongLeaveForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await longLeaveTypeService.updateLongLeaveType(data.id, values);
            toast(
            ...
            await longLeaveTypeService.createLongLeaveType(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>نوع الأجازة الطويلة</FormLabel>
                              <FormControl>
                                 <Input placeholder='الأجازة الطويلة' type='text' {...field} />
                              </FormControl>
