...
import { IAssignmentType } from '../page';
...
type Props = {
   data?: IAssignmentType;
   icon?: React.ReactNode;
   title: string;
   variant?: 'ghost' | 'outline' | 'default' | 'destructive' | 'link';
};

const AssignmentTypeForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await typeOfAssignmentService.updateTypeOfAssignment(data.id, values);
            toast(
            ...
            await typeOfAssignmentService.createTypeOfAssignment(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
                     ...
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              ...
                              <FormControl>
                                 <Input placeholder='نوع التكليف' type='text' {...field} />
                              </FormControl>
