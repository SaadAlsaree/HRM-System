...
import { IAcademicSpecialization } from './academic-specialization-table';
...
type Props = {
   data?: IAcademicSpecialization;
   icon?: React.ReactNode;
   title: string;
   variant?: 'ghost' | 'outline' | 'default' | 'destructive' | 'link';
};

const AcademicSpecializationForm = ({ data, title, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await academicFieldService.putAcademicFieldById(data.id, values.name);
            toast(
            ...
            await academicFieldService.createAcademicField(values.name);
            toast(
               <pre className=' w-[340px] rounded-md'>
...
export default AcademicSpecializationForm;
