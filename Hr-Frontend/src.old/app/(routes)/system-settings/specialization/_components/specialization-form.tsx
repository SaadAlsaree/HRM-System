...
import { IPreciseAcademicField } from '../page';
...
type Props = {
   data?: IPreciseAcademicField;
   icon?: React.ReactNode;
   title: string;
   variant?: 'ghost' | 'outline' | 'default' | 'destructive' | 'link';
};
const SpecializationForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await preciseAcademicFieldService.updatePreciseAcademicField(data.id, values);
            toast(
            ...
            await preciseAcademicFieldService.createPreciseAcademicField(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
                     ...
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              ...
                              <FormControl>
                                 <Input placeholder='التخصص' type='text' {...field} />
                              </FormControl>
