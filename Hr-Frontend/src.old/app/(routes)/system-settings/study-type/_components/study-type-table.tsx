...
const StudyTypeTable = ({ columns, data }: Props) => {
   const router = useRouter();
   //Handel Update status
   const handleStatusChange = async (value: string | number | null, id: string | number | null) => {
      try {
         const response = await studyTypeService.patchStudyType({ id, statusId: value });
               ...
               <TableRow key={item.id}>
                  ...
                  <TableCell>
                     <div className='flex items-center gap-2'>
                        <StudyTypeForm title='' icon={<Settings2 className='h-4 w-4' />} data={item} variant='ghost' />
                     </div>
                  </TableCell>
               </TableRow>
            ))}
