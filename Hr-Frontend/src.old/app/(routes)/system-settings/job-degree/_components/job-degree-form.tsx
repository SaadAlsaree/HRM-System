...
const JobDegreeForm = ({ title, data, icon, variant }: Props) => {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try {
         if (data) {
            await jobDegreeService.updateJobDegree(data.id, values);
            toast(
            ...
            await jobDegreeService.createJobDegree(values);
            toast(
               <pre className=' w-[340px] rounded-md'>
               ...
               <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4 py-2' autoComplete='off'>
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>عنوان الدرجة</FormLabel>
                              <FormControl>
                                 <Input placeholder='الدرجة' type='text' {...field} />
                              </FormControl>
                     ...
                     <FormField
                        ...
                        render={({ field }) => (
                           <FormItem>
                              <FormLabel>مقدار الزيادة</FormLabel>
                              <FormControl>
