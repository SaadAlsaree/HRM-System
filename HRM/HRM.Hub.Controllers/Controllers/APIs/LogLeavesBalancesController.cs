...
using HRM.Hub.Application.Features.LogLeavesBalanceHandlers.Queries.GetLogLeavesBalanceExcelFile;
...
public sealed class LogLeavesBalancesController : Base<LogLeavesBalancesController>
{
    ...
    public LogLeavesBalancesController(IMediator mediator, ILogger<LogLeavesBalancesController> logger) : base(logger)
    {
        _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));
    }
    [ServiceFilter(typeof(LogActionArguments))]
    [HttpGet]
    [ProducesResponseType(typeof(Response<IEnumerable<GetAllLogLeavesBalancesViewModel>>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Response<IEnumerable<GetAllLogLeavesBalancesViewModel>>>> GetList([FromQuery] GetAllLogLeavesBalancesQuery query)
    {
    ...
    [ServiceFilter(typeof(LogActionArguments))]
    [HttpGet("{id:Guid}")]
    [ProducesResponseType(typeof(Response<GetLogLeavesBalanceByIdViewModel>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Response<GetLogLeavesBalanceByIdViewModel>>> GetById(Guid id)
    {
        return await Okey(() => _mediator.Send(new GetLogLeavesBalanceByIdQuery
        {
            Id = id
        }));
    }
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Response<byte[]>>> ExportFile([FromQuery] GetLogLeavesBalanceExcelFileQuery query)
    {
        try
        {
